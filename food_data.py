...
class FoodData:
    def __init__(self, food_data_loc, verbose):
        ...
        if os.path.exists(self.food_data_loc) and self.food_data_loc[-4:] == ".csv":
            ...
            print("WARNING: Food data CSV file " + self.food_data_loc + " is invalid, skipping food analysis.")
            self.to_print = False
            return
            ...
            with open(food_data_loc, "r") as f:
                ...
                for row in reader:
                    ...
                    for diet in danger:
                        if diet == "":
                            continue
                        if not diet in self.danger_diets:
                    ...
                    for diet in warning:
                        if diet == "":
                            continue
                        if not diet in self.danger_diets:
                            if not diet in self.warning_diets:
                                self.warning_diets[diet] = 0
                            self.warning_diets[diet] += 1
                    if key in self.foods:
                        self.foods[key]["count"] += size
                    else:
                        self.foods[key] = {"name": name, "prep": prep, "category": None, "ok": ok, "warning": warning, "danger": danger, "count": size, "unknown": True}
                    date_str = row[0]
                    try:
                    ...
                    if date == None:
                        if verbose:
                            print("Error collecting date from food data record for " + name)
                    elif not date in self.dates_recorded:
                        self.dates_recorded.append(date)
                    if time == None:
                        if verbose:
                            print("Error collecting date and time from food data record for " + name)
                    elif not time in self.meal_times:
                        self.meal_times.append(time)
                    self.record_count += 1
        except Exception as e:
            if verbose:
                print(e)
            print("WARNING: Failed to parse food data CSV file " + self.food_data_loc + ", ensure the file is consistent with sample - skipping food analysis.")
            self.to_print = False
            return
        
        self.meal_times.sort()
        self.avg_meals_per_day = round(len(self.meal_times) / len(self.dates_recorded), 1)
        self.to_print = True
    ...
    def save_most_common_foods_chart(self, graph_cutoff: int, base_dir: str):
        ...
        self.food_labels = [food[:30] for food in sorted(food_to_plot.keys(), key=lambda f: food_to_plot[f])]
        self.food_counts = [count for count in sorted(food_to_plot.values())]

        fig, ax = plt.subplots(1)
        ax.barh(self.food_labels[-graph_cutoff:], self.food_counts[-graph_cutoff:])
        ax.set_xscale('log')
        plt.tight_layout()
    ...
    def get_top_n_danger_diets(self, n: int):
        top_n = []
        for diet in sorted(self.danger_diets, key=lambda d: self.danger_diets[d], reverse=True):
            if len(top_n) == n:
                break
            top_n.append(diet + " (" + str(self.danger_diets[diet]) + " records)")
        return top_n

    def get_top_n_warning_diets(self, n):
        top_n = []
        for diet in sorted(self.warning_diets, key=lambda d: self.warning_diets[d], reverse=True):
            if len(top_n) == n:
                break
            top_n.append(diet + " (" + str(self.warning_diets[diet]) + " records)")
        return top_n
